...
blueprint = Blueprint("multi_hand_tracking", __name__)


@blueprint.route("/", methods=["POST"])
def init_service():
    """Receives the video file sent by the client and applies the
       Mediapipe framework.

    Returns:
        multimedia file -- Returns a video file (.mp4) processed by Mediapipe.
        function -- Removes the input video file (provided by the customer)
        and the output file (generated by Mediapipe).
    """
    ...
    try:
        logging.info("Applying Multi Hand Tracking on video-input.mp4.")
        mediapipe.multi_hand_tracking(request_file_path, response_file_path)
